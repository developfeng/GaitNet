...
def casiab_processor(train_num, im_path, gt_path):
    casiab_dic = './casiab_dic_tr%d'%train_num
    if not os.path.exists(casiab_dic):
        im_list = []
        gt_list = []
        labels = []
        ...
        for id in id_list[:train_num]:
            for con in condition_list:
                for ang in angle_list:
                    pic_im_list = np.sort(os.listdir(os.path.join(im_path, id, con, ang)))
                    pic_gt_list = np.sort(os.listdir(os.path.join(gt_path, id, con, ang)))
                    if len(pic_im_list)>10 and len(pic_im_list)==len(pic_gt_list):
                        im_dir = os.path.join(im_path, id, con, ang)
                        gt_dir = os.path.join(gt_path, id, con, ang)
...
class GaitNet_Seg(caffe.Layer):
    ...
    def setup(self, bottom, top): 
        ...
        self.labels, self.im_list, self.gt_list = casiab_processor(self.train_num, self.im_path, self.gt_path)
        self.idx = 0
        self.num_chn = 6
