...
def outdoor_processor(train_num, im_path, gt_path):
    outdoor_gait_dic = './outdoor_gait_dic'
    if not os.path.exists(outdoor_gait_dic):
        im_list = []
        gt_list = []
        labels = []
        ...
        mypickle(outdoor_gait_dic, dic, compress=False)
    # load the saved data (to resume)
    else:
        dic = myunpickle(outdoor_gait_dic)
        im_list = dic['im_list']
        gt_list = dic['gt_list']
        labels = dic['labels']
...
class GaitNet_Seg(caffe.Layer):
    ...
    def setup(self, bottom, top): 
        ...
        self.labels, self.im_list, self.gt_list = outdoor_processor(self.train_num, self.im_path, self.gt_path)
        self.idx = 0
        self.num_chn = 6
