...
if __name__ == '__main__':
    pass
    data_path = '/data/Outdoor-Gait/im/'
    model_config = './deploy_gaitnet.prototxt'
    model_data = '../models/outdoor-gait/gaitnet-sia_iter_30000.caffemodel'
    test_id = 69
    txt_name='./outdoor_results_tr%d.txt'%test_id
    ...
    gait_fea = './feat/outdoor_fea_tr%d' %test_id
    if not os.path.exists(gait_fea):
        ### Extract features of gallery. 
        condition_list = g_seq_list
        gallery_features = n.zeros((3,len(scene_list)*len(g_seq_list)*(138-test_id),256),n.single)
        gallery_labels = n.zeros((3,len(scene_list)*len(g_seq_list)*(138-test_id)),n.int)
        gallery_valid = n.zeros(3,n.int)
        cont_cloth = -1
        for cloth in cloth_list:
            cont_cloth +=1
            cont_seq = -1
            for id in id_list[test_id:]:
                for scene in scene_list:
                    for con in condition_list:
                        this_dir = os.path.join(data_path, id, '%s_%s_%s_L_090_%s' %(id, scene, cloth, con))
                        if not os.path.exists(this_dir):
                            this_dir = os.path.join(data_path, id, '%s_%s_%s_L_090_%s' %(id, scene[-1], cloth, con))
        ...
        probe_features = n.zeros((3,len(scene_list)*len(p_seq_list)*(138-test_id),256),n.single)
        probe_labels = n.zeros((3,len(scene_list)*len(p_seq_list)*(138-test_id)),n.int)
    ...
    similarity = n.zeros((3,3),n.single)
    fall.write('Prob\Gallery\t')
    for cloth in cloth_list:
        fall.write('%s\t' %cloth)
    for probe in xrange(3):
        fall.write('\n')
